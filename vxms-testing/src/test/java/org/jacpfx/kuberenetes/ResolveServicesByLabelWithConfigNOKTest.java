...
public class ResolveServicesByLabelWithConfigNOKTest extends VertxTestBase {
  ...
  public void initService() {
    ...
    server
        .expect()
        .withPath(
            "/api/v1/namespaces/default/services?labelSelector=service1x.name%3Dservice1x.value")
        .andReturn(200, new ServiceListBuilder().build())
        .times(1);
    server
        .expect()
        .withPath(
            "/api/v1/namespaces/default/services?labelSelector=service2x.name%3Dservice2x.value")
        .andReturn(200, new ServiceListBuilder().build())
        .times(1);
  }
            ...
            resp -> {
              resp.bodyHandler(
                  body -> {
                    ...
                    vertx.runOnContext(
                        context -> {
                          failed.set(!response.equalsIgnoreCase("null/null"));
  ...
  public class WsServiceOne extends VxmsEndpoint {
    ...
    public Config config;
    @ServiceName
    @WithLabel(name = "${service1x.name}", value = "${service1x.value}")
    private String service1;
    @ServiceName
    @WithLabel(name = "${service2x.name}", value = "${service2x.value}")

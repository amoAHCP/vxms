 ...
 */
public class RESTJerseyClientEventObjectResponseAsyncTest extends VertxTestBase {
  ...
  public class WsServiceTwo extends VxmsEndpoint {
    ...
    public void complexByteResponse(RestHandler reply) {
      ...
      reply.
          ...
          blocking().
          send("hello", "welt").
          mapToObjectResponse(handler -> new Payload<>(handler.result().body().toString()),
    ...
    public void simpleByteNoConnectionRetryErrorResponse(RestHandler reply) {
      reply.eventBusRequest().
          blocking().
          send("error", "1").
          mapToObjectResponse(handler -> new Payload<>(handler.result().body().toString() + "1"),
              new ExampleByteEncoder()).
          retry(3).
          execute();
    }

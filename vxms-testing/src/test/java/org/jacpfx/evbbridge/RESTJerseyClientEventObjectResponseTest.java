  ...
  public class WsServiceTwo extends VxmsEndpoint {
    ...
    public void complexByteResponse(RestHandler reply) {
      ...
      reply
          ...
          .mapToObjectResponse(
              (handler, replyFuture) ->
                  replyFuture.complete(new Payload<>(handler.result().body().toString())),
    ...
    public void complexByteErrorResponse(RestHandler reply) {
      reply
          ...
          .mapToObjectResponse(
              ...
              new ExampleByteEncoder())
          .onFailureRespond(
              (error, future) -> future.complete(new Payload<>(error.getMessage())),
              new ExampleByteEncoder())
          .execute();
    }
    ...
    public void simpleByteNoConnectionErrorResponse(RestHandler reply) {
      reply
          ...
          .mapToObjectResponse(
              ...
              new ExampleByteEncoder())
          .onFailureRespond(
              (t, c) -> c.complete(new Payload<>("no connection")), new ExampleByteEncoder())
          .execute();
    }

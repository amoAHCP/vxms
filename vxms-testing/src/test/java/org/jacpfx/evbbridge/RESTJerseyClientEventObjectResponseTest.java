...
public class RESTJerseyClientEventObjectResponseTest extends VertxTestBase {
  ...
  public void complexByteResponseTest() throws InterruptedException {
    ...
    WebTarget target = client.target("http://" + HOST + ":" + PORT2)
        .path("/wsService/complexByteResponse");
  ...
  public void complexByteErrorResponseTest() throws InterruptedException {
    ...
    Client client = ClientBuilder.newClient();
    WebTarget target = client.target("http://" + HOST + ":" + PORT2)
        .path("/wsService/complexByteErrorResponse");
    Future<byte[]> getCallback = target.request(MediaType.APPLICATION_JSON_TYPE).async()
        .get(new InvocationCallback<byte[]>() {
  ...
  public class WsServiceTwo extends VxmsEndpoint {
    ...
    public void complexByteResponse(RestHandler reply) {
      ...
      reply.
          eventBusRequest().
          send("hello", "welt").
          mapToObjectResponse((handler, replyFuture) -> replyFuture
                  .complete(new Payload<>(handler.result().body().toString())),
    ...
    public void complexByteErrorResponse(RestHandler reply) {
      reply.eventBusRequest().
          ...
          onFailureRespond((error, future) -> future.complete(new Payload<>(error.getMessage())),
              new ExampleByteEncoder()).
          execute();
    }
    ...
    public void simpleByteNoConnectionErrorResponse(RestHandler reply) {
      reply.eventBusRequest().
          ...
          mapToObjectResponse((handler, replyFuture) -> replyFuture
                  .complete(new Payload<>(handler.result().body().toString())),
              new ExampleByteEncoder()).
          onFailureRespond((t, c) -> c.complete(new Payload<>("no connection")),
              new ExampleByteEncoder()).
          execute();
    }

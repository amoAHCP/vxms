...
public class RESTJerseyClientEventByteResponseTest extends VertxTestBase {
  ...
  public void complexByteResponseTest() throws InterruptedException {
    ...
    WebTarget target =
        client.target("http://localhost:" + PORT2).path("/wsService/complexByteResponse");
  ...
  public void complexByteErrorResponseTest() throws InterruptedException {
    ...
    Client client = ClientBuilder.newClient();
    WebTarget target =
        client.target("http://localhost:" + PORT2).path("/wsService/complexByteErrorResponse");
    Future<byte[]> getCallback =
        target
            .request(MediaType.APPLICATION_JSON_TYPE)
            .async()
  ...
  public class WsServiceTwo extends VxmsEndpoint {
    ...
    public void simpleByteNoConnectionErrorResponse(RestHandler reply) {
      reply
          ...
          .send("hello1", "welt")
          .mapToByteResponse(
              (handler, replyFuture) -> replyFuture.complete((byte[]) handler.result().body()))
          .onFailureRespond(
              (t, c) -> {
                try {

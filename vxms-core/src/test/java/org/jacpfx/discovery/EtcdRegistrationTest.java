...

{
  ...
  public void testEtcdDiscoveryClientAndConnect() {
    ...
    final DiscoveryClient client = DiscoveryClient.createClient(vertx, new HttpClientOptions(),
        new JsonObject().put("etcd-host", "127.0.0.1").put("etcd-port", "4001")
            .put("etcd-domain", "etcdAwareTest"));
        ...
        vertx.executeBlocking(blocking -> {
          client.find(SERVICE_REST_GET).onSuccess(val -> {
            ...
            HttpClientRequest request = httpclient
                .getAbs(val.getServiceNode().getUri().toString() + "/simpleRESTEndpoint/", resp -> {
      ...
      if (connected.succeeded()) {
        vertx.executeBlocking(blocking -> {
          client.find(SERVICE_REST_GET_2).onSuccess(val -> {
            ...
            HttpClientRequest request = httpclient
                .getAbs(val.getServiceNode().getUri().toString() + "/simpleRESTEndpoint/hello",
                    resp -> {
                      if (resp.statusCode() == 200) {
                        resp.bodyHandler(body -> {
                          System.out.println("Got a createResponse: " + body.toString());
  ...
  public class EtcdAwareServiceA extends VxmsEndpoint {
    ...
    public void simpleRESTEndpoint(RestHandler reply) {
      ...
      client.find(SERVICE_REST_GET_3).onSuccess(node -> {
        HttpClientRequest request = httpclient.
            getAbs(
                node.getServiceNode().getUri().toString() + "/simpleRESTEndpoint/" + reply.request()
                    .param("value"), resp -> {
                  if (resp.statusCode() == 200) {
                    resp.bodyHandler(body -> {
                      System.out
                          .println("Got a createResponse in EtcdAwareServiceA: " + body.toString());
                      reply.response().stringResponse((future) -> future.complete(body.toString()))
                          .execute();
                    });
                  } else {
                    // do something
  ...
  public class EtcdAwareServiceB extends VxmsEndpoint {
    ...
    public void simpleRESTEndpoint(RestHandler reply) {
      System.out.println("stringResponse: " + reply);
      reply.response()
          .stringResponse((future) -> future.complete("test-123" + reply.request().param("value")))
          .execute();
    }

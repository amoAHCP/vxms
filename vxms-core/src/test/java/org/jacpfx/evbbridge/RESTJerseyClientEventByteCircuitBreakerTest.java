 ...
 */
public class RESTJerseyClientEventByteCircuitBreakerTest extends VertxTestBase {
    ...
    public void simpleSyncNoConnectionErrorResponse(RestHandler reply) {
      ...
      reply.eventBusRequest().
          ...
          mapToByteResponse((handler, future) -> {
            ...
            future
                .complete(Serializer.serialize(new Payload<>(handler.result().body().toString())));
          }).
          onError(error -> {
          ...
          retry(3).
          onFailureRespond(
              (t, c) -> c.complete(Serializer.serialize(new Payload<>(t.getMessage())))).
          execute();

...
- handler.response().onFailureRespond((error,future)-> future.complete("")) : provides a terminal function, that will be called when no other retries or error handling is possible and defines the final response when the default one fails.

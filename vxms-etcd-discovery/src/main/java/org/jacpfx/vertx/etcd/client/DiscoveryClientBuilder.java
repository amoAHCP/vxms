...
public class DiscoveryClientBuilder implements DiscoveryClientSpi<DiscoveryClientEtcd> {
  ...
  public DiscoveryClientEtcd getClient(AbstractVerticle verticleInstance) {
    if (verticleInstance.getClass().isAnnotationPresent(EtcdClient.class)) {
      ...
      final HttpClientOptions clientOptions = connection
          .getClientOptions(verticleInstance.config());
      ...
      final boolean secure = verticleInstance.config()
          .getBoolean("etcd-secure", clientOptions.isSsl());
      final String prefix = secure ? HTTPS : HTTP;
      final URI fetchAll = URI.create(
          prefix + etcdHost + ":" + client.port() + ETCD_BASE_PATH + domain + "/?recursive=true");
      return new DiscoveryClientEtcd(Vertx.vertx(), clientOptions, client.domain(), fetchAll,
          etcdHost, etcdPort);
    } else {
      throw new MissingResourceException("missing @EtcdClient annotation",
          verticleInstance.getClass().getName(), "");
    }
  }
  ...
  public DiscoveryClientEtcd getClient(Vertx vertx, HttpClientOptions clientOptions,
      JsonObject config) {
    ...
    final String prefix = secure ? HTTPS : HTTP;
    final URI fetchAll = URI
        .create(prefix + host + ":" + port + ETCD_BASE_PATH + domain + "/?recursive=true");
    return new DiscoveryClientEtcd(vertx, clientOptions, domain, fetchAll, host,

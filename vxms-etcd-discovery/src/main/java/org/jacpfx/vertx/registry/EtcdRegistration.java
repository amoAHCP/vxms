...
public class EtcdRegistration {
  ...
  private static final List<Integer> SUCCESS_CODES = Collections
      .unmodifiableList(Arrays.asList(200, 201, 403));
  private static final String CONTENT_TYPE = HttpHeaders.CONTENT_TYPE.toString();
  private static final String APPLICATION_X_WWW_FORM_URLENCODED = HttpHeaders.APPLICATION_X_WWW_FORM_URLENCODED
      .toString();
   ...
   */
  private EtcdRegistration(Vertx vertx, HttpClientOptions clientOptions, String etcdHost,
      int etcdPort, int ttl, String domainname, String servicename, String host, int port,
      String contextRoot, boolean secure, String nodename) {
    this.vertx = vertx;
    this.ttl = ttl;
    ...
    this.fetchAll = URI.create(clientOptions.isSsl() ? HTTPS
        : HTTP + etcdHost + ":" + etcdPort + ETCD_BASE_PATH + domainname + "/?recursive=true");
  ...
  private static String cleanPath(String path) {
    ...
    final int len = path01.length();
      if (path01.charAt(len - 1) == '/') {
          return path01.substring(0, len - 1);
      }
    return path01;
  }

  public static VertxBuilder buildRegistration() {
    return vertx -> clientOptions -> etcdHost -> etcdPort -> ttl -> domainname -> servicename -> serviceHost -> servicePort -> contextRoot -> secure -> nodename -> new EtcdRegistration(
        vertx, clientOptions, etcdHost, etcdPort, ttl, domainname, servicename, serviceHost,
        servicePort, contextRoot, secure, nodename);
  }
  ...
  public void connect(AsyncResultHandler<DiscoveryClient> asyncResultHandler) {
    connectToEtcd(vertx, domainname, servicename, nodename, ttl, contextRoot, host, port, secure,
        asyncResultHandler);
  }
  ...
  private void connectToEtcd(Vertx vertx, String _domainname, String _servicename, String _nodename,
      int _ttl, String _contextRoot, String _host, int _port, boolean _secure,
      AsyncResultHandler<DiscoveryClient> _asyncResultHandler) {
    createServiceNode(_servicename,
        pathCreated -> {
          //403 means the directory already existed
            ...
            {
                createInstanceNode(_domainname, _servicename, _nodename,
                    "value=" + Json.encode(new NodeMetadata(_contextRoot, _host, _port, _secure))
                        + "&ttl=" + ttl,
                    nodeCreated -> {
                        if (SUCCESS_CODES.contains(nodeCreated.statusCode()) || 403 == nodeCreated
                            .statusCode()) {
                            retrieveKeys(root -> {
                                putRootToCache(root);
                                startNodeRefresh(vertx, _domainname, _servicename, nodename, _ttl,
                                    _contextRoot, _host, _port, _secure);
                                _asyncResultHandler.handle(Future.factory.succeededFuture(
                                    new DiscoveryClientEtcd(vertx, options, _domainname,
                                        fetchAll)));
                            });
                        } else {
                            LOG.error(
                                "Unable to create node (" + nodeCreated.statusCode() + ") "
                                    + nodeCreated
                                    .statusMessage());
                            _asyncResultHandler.handle(Future.factory.failureFuture(
                                ("Unable to create node node (" + nodeCreated.statusCode() + ") "
                                    + nodeCreated.statusMessage())));
                        }

                    }, _asyncResultHandler);
            } else {
                LOG.error(
                    "Unable to create service node (" + pathCreated.statusCode() + ") "
                        + pathCreated
                        .statusMessage());
                _asyncResultHandler.handle(Future.factory.failureFuture(
                    "Unable to create service node (" + pathCreated.statusCode() + ") "
  ...
  private void startNodeRefresh(Vertx vertx, String _domainname, String _servicename,
      String _nodename, int ttl, String _contextRoot, String _host, int _port, boolean _secure) {
    LOG.info("Succeeded registering");
    // ttl in s setPeriodic in ms
    vertx.setPeriodic((ttl * 1000) - 900,
        refresh -> createInstanceNode(_domainname, _servicename, _nodename,
            "value=" + Json.encode(new NodeMetadata(_contextRoot, _host, _port, _secure)) + "&ttl="
                + ttl, refreshed -> {
              if (refreshed.statusCode() != 200) {
                LOG.error("Unable to refresh node (" + refreshed.statusCode() + ") " + refreshed
                    .statusMessage() + "  HOST: " + getOptions().getDefaultHost() + " PORT: "
                    + getOptions().getDefaultPort());
              } else {
  ...
  private void createServiceNode(String serviceName, Handler<HttpClientResponse> responseHandler,
      AsyncResultHandler<DiscoveryClient> asyncResultHandler) {
        ...
        .exceptionHandler(error -> {
          error.printStackTrace();
            if (asyncResultHandler != null) {
                asyncResultHandler.handle(Future.factory.failedFuture(error));
            }
  ...
  private void createInstanceNode(String domainname, String serviceName, String name, String data,
  ...
  private void deleteInstanceNode(String domainname, String serviceName, String name,
      Handler<HttpClientResponse> responseHandler,
      AsyncResultHandler<DiscoveryClient> asyncResultHandler) {
